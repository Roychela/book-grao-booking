...
def meetingbooker():
    ...
    for booking in bookings:
        meetingreturn=dict()
        meetingreturn['title']=booking.title
        # meetingreturn['team']=Team.query.filter_by(id=meeting.teamId).first().teamName
        meetingreturn['room']=Room.query.filter_by(id=booking.roomId).first().roomName
        meetingreturn['booker']=User.query.filter_by(id=booking.bookerId).first().fullname
        meetingreturn['date']=booking.date.date()
        meetingreturn['time']=f'{booking.startTime} to {booking.endTime}'
        meetingreturns.append(meetingreturn)
    return render_template('meetingbooker.html',bookings=meetingreturns)
...
def book():
    ...
    if form.validate_on_submit():
        ...
        meetingcollisions=Booking.query.filter_by(date=datetime.combine(form.date.data,datetime.min.time())).filter_by(roomId=form.rooms.data).all()
        ...
        meeting=Booking(title=form.title.data,roomId=room.id,bookerId=booker.id,date=form.date.data,startTime=form.startTime.data,endTime=endTime,duration=form.duration.data)
...
def roomavailable():
    ...
    if form.validate_on_submit():
        meetings=Booking.query.filter_by(date=datetime.combine(form.date.data,datetime.min.time())).all()
        roomsOccupied=set()
...
def roomoccupation():
    ...
    if form.validate_on_submit():
        ...
        for room in rooms:
            ...
            for hour in hours:
                meetings=Booking.query.filter_by(date=datetime.combine(form.date.data,datetime.min.time())).filter_by(roomId=room.id).all()
